...
import {
  Cache, CacheExchangeOpts, QueryInput
} from "@urql/exchange-graphcache";
import Container from 'typedi';
import { dedupExchange, errorExchange, fetchExchange, ssrExchange } from "urql";
import { ApiConfigOptions, AppConfigOptions } from '../../core/config/config';
import { isServerSide } from "../../core/utils";
import { TOKEN_KEY } from '../components/AuthContext';
import {
  LoginMutation,
  MeDocument,
  MeQuery,
...
export const UrqlClientConfig = () => {  
  ...
  const apiConfig:ApiConfigOptions = Container.get('ApiConfigOptions');
  ...
  return {
    url: apiConfig.endpoints['graphql'],
    fetchOptions: {
      credentials: "include" as const,
    },
    // TODO: provide a way for the exchanges to be configured by the app.
    exchanges: [
      devtoolsExchange,
      dedupExchange,    
      urqlErrorExchange(),
